...
export const CommitViewReducer: Reducer<CommitViewState> = (
	state: CommitViewState = defaultCommitViewState,
	action: Action
): CommitViewState => {
	if (CommitViewActions.setCommitViewErrorMap.match(action)) {
		return {
			...state,
	...
	if (CommitViewActions.setLoadingPrintModelResponse.match(action)) {
		return {
			...state,
			printModel: action.payload.printModel,
			logPersistentEntries: action.payload.logPersistentEntries,
		};
	}

	if (CommitViewActions.setCommitInteractionRows.match(action)) {
		return {
			...state,
