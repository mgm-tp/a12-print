...
function* handleSetSerializePrintModelResultSaga(action: PayloadAction<MarshallerResult<PrintModel, PrintModelDTO>>) {
	...
	if (serializeRes.report.relevantPaths.length) {
		yield* put(
			ValidationActions.appendErrors({
				errorMap: serializeRes.report.errorMap,
			})
		);
	} else {
		yield* put(ValidationActions.setErrorMap(serializeRes.report.errorMap));
	}
	const validationState = yield* select(ValidationSelectors.validationState);
	requestApi.onValidationStateChange(validationState);

	if (serializeRes.result) {
		requestApi.deserializePrintModel(JSON.stringify(serializeRes.result), serializeRes.report.relevantPaths);
	}
}
