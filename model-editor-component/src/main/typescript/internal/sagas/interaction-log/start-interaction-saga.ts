...
type InteractionSaga = (
	action: Action<InteractionLogActions.StartPayload>,
...
function takeInteractionStart(
	ms: number,
	pattern: (action: AnyAction) => boolean,
...
export function* startInteractionSaga(): SagaIterator {
...
function* getRegionId(region: InteractionRegion) {
	const { refId, placeableRefId } = (yield* select(PrintEngineSelectors.currentDetailData)) || {};
	switch (region) {
		...
		case TextRegion.TEXT_FROM_CALCULATION:
			return yield* call(getTextRegionIdSaga);
		case GlobalRegion.FORM:
			return refId || placeableRefId;
		case StageRegion.DEFAULT:
		case StageRegion.LAYOUT:
		case StageRegion.READING_ORDER:
			return yield* select(PrintEngineSelectors.currentDetailDataId);
		...
		default:
			return (yield* select(PrintEngineSelectors.sidebar)).selectedItem;
	}
}
...
function* getListingRegionIdSaga(region: InteractionRegion) {
	...
	return getListingRegionId(region, currentDetailData);
}

function* getTableRegionIdSaga() {
	const currentDetailData = yield* select(PrintEngineSelectors.currentDetailData);
	return getTableRegionId(currentDetailData);
}

function* getTextRegionIdSaga() {
	const entityElement = yield* select(PrintEngineSelectors.textEntityElement);

	return entityElement?.id;
}
