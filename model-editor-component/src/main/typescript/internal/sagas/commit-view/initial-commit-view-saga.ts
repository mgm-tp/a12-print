...
function* handleInitialCommitViewSaga() {
	...
	const printModelHeader = yield* select(PrintEngineSelectors.printHeader);
	const loadPrintModelResponse = yield* call(requestApi.loadPrintModel, printModelHeader.id);

	if (loadPrintModelResponse?.printModel) {
		yield* put(
			CommitViewActions.setLoadingPrintModelResponse({
				printModel: loadPrintModelResponse.printModel,
				logPersistentEntries: loadPrintModelResponse.logPersistentEntries,
			})
		);
	}

	const commitInteractionRows = yield* call(createCommitInteractionRows);
	yield* put(CommitViewActions.setCommitInteractionRows(commitInteractionRows));
}
