...
describe("ListItem", () => {
	it("renders correctly with label and icon", () => {
		const { container, queryByText } = renderWithProviders(<ListItem name="foo" iconName="bar" />);
	...
	it("renders correctly without icon", () => {
		const { container, queryByText } = renderWithProviders(<ListItem name="baz" />);

		expect(container.getElementsByClassName("plasma-icon")).toHaveLength(0);
