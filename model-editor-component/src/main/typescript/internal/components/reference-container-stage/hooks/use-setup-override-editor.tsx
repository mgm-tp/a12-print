...
import type { PartialOverride, PartialValidPlaceableReference } from "@com.mgmtp.a12.print/print-model-api/model";
...
export const useSetupOverrideEditor = (overrideElement: PartialOverride) => {
	...
	const numberOfPages = React.useMemo(() => {
		return EditorUtils.calculateNumberOfPages(
			...
			overrideElement?.override?.boundingBox?.elementReferences as PartialValidPlaceableReference[]
		);
	}, [overrideElement?.override?.boundingBox?.elementReferences, editorDimensions.minHeight.value]);
	...
	React.useEffect(() => {
		setElementReferences(
			ElementsUtils.getElementReferences(overrideElement) as ReadonlyArray<PartialValidPlaceableReference>
		);
	}, [overrideElement, setElementReferences]);
