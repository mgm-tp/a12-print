...
export const StyledFloatingButtonContainer = styled.div`
	position: absolute;
	top: 4px;
	right: 4px;
	bottom: 0;
...
export const SwitchAreaOuterContainer = styled.div`
	max-height: ${SWITCH_CASE_MAX_ELEMENT_HEIGHT}px;
	position: relative;
	background-color: #fff;
`;

export const SwitchAreaInnerContainer = styled.div<{
	scale: number;
	dimensions?: DeepPartialRecursive<OverflowDimensions>;
}>`
	border: ${({ scale }) => 1 / scale}px solid #aaa;
