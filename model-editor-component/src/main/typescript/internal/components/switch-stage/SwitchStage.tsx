/*
 * SPDX-License-Identifier: EUPL-1.2 OR LicenseRef-commercial
 *
 * Copyright (c) 2012-2026 mgm technology partners GmbH
 *
 * Dual License
 * ------------
 * This source file is part of the mgm A12 Platform and available under
 * a choice of two different licenses:
 *
 * 1. Open-Source License - EUPL v1.2
 *    You may redistribute and/or modify this file under the terms of the
 *    European Union Public License, version 1.2 - see https://eupl.eu/.
 *
 * 2. Commercial License
 *    Alternatively, you may obtain a commercial license from
 *    mgm technology partners GmbH, that permits use of this software
 *    under different terms (including support and maintenance services).
 *
 *    Please contact a12-license@mgm-tp.com for more information.
 *
 * You must select and comply with exactly one of the above license options.
 *
 * Warranty Disclaimer (applies to either option)
 * ----------------------------------------------
 * THIS SOFTWARE IS PROVIDED "AS IS" AND WITHOUT WARRANTY OF ANY KIND,
 * WHETHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES
 * OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND
 * NON-INFRINGEMENT, EXCEPT WHERE SUCH DISCLAIMERS ARE HELD TO BE
 * LEGALLY INVALID. SEE THE RESPECTIVE LICENSE TEXT FOR DETAILS.
 */
import * as React from "react";
import { useDispatch, useSelector } from "react-redux";
import { nanoid } from "nanoid";

import type { PartialArea } from "@com.mgmtp.a12.print/print-model-api/model";
import { ElementType, PartialSwitch } from "@com.mgmtp.a12.print/print-model-api/model";
import { StageRegion } from "@com.mgmtp.a12.print/print-model-api-utils/a12internal";

import { PrintEngineSelectors } from "../../store/selectors.js";
import { InteractionLogActions, TransactionLogStateActions } from "../../redux/index.js";
import { RESOURCE_KEYS } from "../../localization/index.js";
import { createMmMeasure } from "../../utils/index.js";

import { AddButtonGroup } from "../forms/shared-components/AddButtonGroup.js";
import { BreadcrumbNavigation } from "../breadcrumb-navigation/BreadcrumbNavigation.js";

import { SwitchStageContainer } from "./SwitchStage.styled.js";
import { SwitchCaseList } from "./SwitchCaseList.js";

export const SwitchStage = () => {
	const dispatch = useDispatch();
	const wrapperElement = useSelector(PrintEngineSelectors.currentWrapperContainer);

	if (!wrapperElement || !PartialSwitch.isInstance(wrapperElement)) {
		throw Error(`Expected switch element but got ${wrapperElement}`);
	}

	const onAddClick = React.useCallback(() => {
		const newAreaId = nanoid();
		const newAreaElement: PartialArea = {
			id: newAreaId,
			type: ElementType.Area,
			area: {
				id: nanoid(),
				dimensions: { id: nanoid(), ...wrapperElement.switch?.dimensions, overflowHeight: createMmMeasure(0) },
			},
		};
		dispatch(
			InteractionLogActions.start({
				region: StageRegion.SWITCH,
				description: RESOURCE_KEYS.interaction.form.switchFormContainer.addCase,
				transactionLogActions: [
					TransactionLogStateActions.updatePrintModelElements({
						data: [
							newAreaElement,
							{
								...wrapperElement,
								switch: {
									id: nanoid(),
									...wrapperElement.switch,
									cases: [
										...(wrapperElement.switch?.cases || []),
										{ id: nanoid(), refId: newAreaId },
									],
								},
							},
						],
					}),
				],
			})
		);
	}, [dispatch, wrapperElement]);

	return (
		<SwitchStageContainer>
			<BreadcrumbNavigation />
			<SwitchCaseList switchElement={wrapperElement} />
			<AddButtonGroup onClick={onAddClick} />
		</SwitchStageContainer>
	);
};
