...
export function createSimpleButton({ onClick, icon, title }: SimpleButtonProps) {
	const SimpleButton = () => {
		const localizer = PrintLocalizer.useLocalizer();
		const { disabled } = React.useContext(ToolbarContext);

		return <EditorButtonInternal title={localizer(title)} icon={icon} onClick={onClick} disabled={disabled} />;
	};
	return SimpleButton;
}
