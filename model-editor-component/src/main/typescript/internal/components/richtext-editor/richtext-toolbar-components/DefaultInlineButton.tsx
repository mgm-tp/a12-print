...
const INLINE_BUTTON_MAPPING = {
	BOLD: { icon: "format_bold", labelKey: RESOURCE_KEYS.elementForm.textProperties.bold },
	ITALIC: { icon: "format_italic", labelKey: RESOURCE_KEYS.elementForm.textProperties.italic },
	UNDERLINE: { icon: "format_underline", labelKey: RESOURCE_KEYS.elementForm.textProperties.underline },
};

type StyleName = keyof typeof INLINE_BUTTON_MAPPING;

export function createDefaultInlineButton(styleName: StyleName) {
	const DefaultInlineButton = ({ editorState, setEditorState }: ToolbarButtonInternalProps) => {
		...
		const { icon, labelKey } = INLINE_BUTTON_MAPPING[styleName];
		return (
