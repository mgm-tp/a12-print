...
import { StyledBoundingBoxElements } from "./BoundingBoxElements.styled.js";
...
export const BoundingBoxElements: React.FunctionComponent<Props> = ({ item, children }) => {
	const itemElement = useSelector((state: PrintEngineState) =>
		PrintEngineSelectors.printModelElement(state, item.refId)
	);

	return (
		<StyledBoundingBoxElements itemElement={itemElement} item={item}>
			{children}
		</StyledBoundingBoxElements>
	);
};
