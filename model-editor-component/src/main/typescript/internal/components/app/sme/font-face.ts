...
const getPrefixedFont = (fontFamily: string) => {
	return `${PRINT_FONT_PREFIX}${fontFamily}`;
};

export const createGlobalFontFaces = (fontMap: PrintFontMap | FontResourceMap = DEFAULT_FONTS) => `
		${Object.entries(fontMap)
			.map(
				([name, { url, format }]) =>
					...
					`
				@font-face {
					font-family: "${getPrefixedFont(name)}";
