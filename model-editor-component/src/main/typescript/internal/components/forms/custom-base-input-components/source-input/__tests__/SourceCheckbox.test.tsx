...
describe("SourceCheckbox", () => {
	...
	describe("source toggle display", () => {
		it("shows INPUT and DEFAULT source toggles when no inputSource is set", () => {
			const { getByTitle, queryByTitle } = renderWithProviders(
				<SourceCheckbox label="Bold" onChange={mockOnChange} sourceProperties={makeSourceProps()} />
			);
	...
	describe("checkbox interaction by source", () => {
		it("checkbox is not read-only when source is INPUT", () => {
		...
		it("checkbox is read-only when source is DEFAULT", () => {
			const { getByRole } = renderWithProviders(
				<SourceCheckbox
					label="Bold"
					onChange={mockOnChange}
			...
			// The Checkbox widget renders readonly as disabled on the <input> element
			const checkbox = getByRole("checkbox");
			expect(checkbox).toBeDisabled();
		});

		it("checkbox is read-only when source is INHERITED", () => {
			const { getByRole } = renderWithProviders(
				<SourceCheckbox
					label="Bold"
					onChange={mockOnChange}
