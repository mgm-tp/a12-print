...
export function RowTablePropertyComputation({
	...
	const onAddPropertyComputation = React.useCallback(() => {
		...
		updatePropertyComputations([
			...propertyComputations,
			{ id: propertyCompId } as DeepPartial<RowPropertyComputations>,
		]);
	...
	const componentRenderers = React.useMemo(() => {
		const bodyContentRenderer = (
			props: TableRenderPropsType.BodyContentProps<DeepPartial<RowPropertyComputations>>
		) => {
		...
		const bodyRowRenderer = (props: TableRenderPropsType.BodyRowProps<DeepPartial<RowPropertyComputations>>) => {
			return (
	...
	return (
		<>
			<FormContainerHeadline
				label={localizer(RESOURCE_KEYS.elementForm.listing.rowPropertiesComputations.headline)}
			/>
			<div>
				<Table<DeepPartial<RowPropertyComputations>>
