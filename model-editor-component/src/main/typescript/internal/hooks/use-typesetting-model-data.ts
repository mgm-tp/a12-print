...
export function useTypesettingModelData(modelId?: string) {
	const typesettingModelData = useSelector((state: PrintEngineState) =>
		modelId ? RequestApiSelectors.typesettingModel(state, modelId) : undefined
	);
	const dispatch = useDispatch();

	useEffect(() => {
		if (!typesettingModelData && modelId) {
			dispatch(RequestApiActions.loadTypesettingModel(modelId));
		}
	}, [dispatch, modelId, typesettingModelData]);

	return typesettingModelData;
}
