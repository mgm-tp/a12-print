...
public class TextStyleDependency implements ValueDependency<TextStyle> {
	private String textStyleId;

	public static TextStyleDependency create(Optional<StringInputSource> textStyleInput, InputValueSourceResolver.ReferenceResolver referenceInputSourceResolver) {
		var textStyleId = Constants.DEFAULT_TEXT_STYLE.getId();

		if (textStyleInput.isPresent()) {
			textStyleId = InputValueSourceResolver.getInputValue(textStyleInput.get(), referenceInputSourceResolver).orElse(Constants.DEFAULT_TEXT_STYLE.getId());
		}

		return new TextStyleDependency(textStyleId);
	}
}
