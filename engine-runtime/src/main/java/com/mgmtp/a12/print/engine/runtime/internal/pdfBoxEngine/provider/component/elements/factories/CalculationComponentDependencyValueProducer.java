...
public class CalculationComponentDependencyValueProducer implements PdfBoxDependencyValueProvider<Component, CalculationComponentDependency> {

	@Override
	public ValueFactory<Component> produce(CalculationComponentDependency dependency, PrintJob job, PrintEngine<?> engine, InternalPdfBoxPrintEngineRuntime runtime) {
		final var calculationTrace = dependency.getCalculationTrace();
		final var formattingResult = runtime.provide(new CalculationValueDependency(
			calculationTrace,
			dependency.getPrintDocumentContext()
		)).get();

		return () -> new EntityComponent(
			calculationTrace.getTracedElement().getId(),
			calculationTrace,
