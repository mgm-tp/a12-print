 ...
 */
package com.mgmtp.a12.print.engine.runtime.internal.engine.provider.element.markup.container;
...
@Builder
public record ContainerHtmlTemplateParameters(
	PrintModelElement element,
	PlaceableReference placeableReference,
	Dimensions dimensions,
	List<MarkupCollectorKey> childElements
) implements HtmlTemplateParametersWithMarkups {
	public Optional<BorderProperties> borderProperties() {
		if (element instanceof Area) {
			return ((Area) element).getBorderProperties();
		} else if (element instanceof BoundingBox) {
			return ((BoundingBox) element).getBorderProperties();
		}
		return Optional.empty();
	}

	@Override
	public List<MarkupCollectorKey> getChildElements() {
