...
import com.mgmtp.a12.print.engine.runtime.internal.manager.compiler.layout.componentTrees.*;
	...
	private static class PlaceableReferenceContainerVisitor implements PrintModelVisitor {
		...
		public TraversalCommand visitContainer(
			...
			if (container instanceof PlaceableReferenceContainer placeableReferenceContainer) {
				...
				final var parentTopLevelReferenceContainerTrace = path.findParentTopLevelReferenceContainer().orElseThrow(() -> new PrintCompilerException(
					"The nested container {} does not have a segment/section as parent", elementReferenceContainer.getId()
				));
		...
		public TraversalCommand visitDINTemplate(ModelSegment dinTemplate, PrintModelPath path) {
			...
			if (relatedSegmentTrace == null) {
				throw new PrintCompilerException("The corresponding segment was not visited before");
			}
		...
		private void addReferenceAndNestedContainerComponentTree(
			...
			final var parentTopLevelReferenceContainerTrace = placeableReferenceContainerTrace.getPath().findParentTopLevelReferenceContainer().orElseThrow(() -> new PrintCompilerException(
				"The nested container {} does not have a segment/section as parent", placeableReferenceContainer.getId()
			));
		...
		private void addNestedContainerComponentTree(
			...
			if (reference == null && parentSwitchElement == null) {
				throw new PrintCompilerException("Each nested container needs to have a related placeable reference");
			}
			...
			switch (container) {
				case Area area -> {
					...
					if (areaRef == null) {
						...
						if (switchRef == null) {
							throw new PrintCompilerException("Each Area inside a Switch needs to have a related placeable reference");
						}
				...
				default -> throw new PrintException("The nested container {} does not have a component", container);
			}
		}
