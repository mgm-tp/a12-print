...
public class ReferenceMarkupResultDependency implements ValueDependency<MarkupResult> {
	...
	public static Stream<ReferenceMarkupResultDependency> ofContainerReferences(
		...
		return path.getTracedElement().getReferences().stream()
                   .map(e -> new ReferenceMarkupResultDependency(path.createDescendent(e), document));
	}
