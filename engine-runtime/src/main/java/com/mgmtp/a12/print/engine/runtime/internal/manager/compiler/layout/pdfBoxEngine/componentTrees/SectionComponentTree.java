...
public class SectionComponentTree extends ContainerComponentTree {
	...
	public ValueFactory<PageBreakInterruptResult<ComponentTreeResult>> produce(ComponentTreeDependency dependency, PrintJob job, PrintEngine<?> engine, InternalPdfBoxPrintEngineRuntime runtime) {
		...
		if (!(documentHandle instanceof SectionDocumentHandle sectionDocumentHandle)) {
			throw new PrintException("Only SectionDocumentHandles are supported for sections");
		}

		return () -> PageBreakInterruptResult.of(sectionDocumentHandle.renderSection(() -> ContainerTree.builder()
