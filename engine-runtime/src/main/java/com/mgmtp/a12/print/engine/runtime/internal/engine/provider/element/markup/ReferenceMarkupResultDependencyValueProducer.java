 ...
 */
package com.mgmtp.a12.print.engine.runtime.internal.engine.provider.element.markup;
...
@Slf4j
public class ReferenceMarkupResultDependencyValueProducer implements PdfDependencyValueProvider<MarkupResult, ReferenceMarkupResultDependency> {

	@Override
	public ValueFactory<MarkupResult> produce(ReferenceMarkupResultDependency dependency, PrintJob job, PrintEngine<?> engine, InternalPdfPrintEngineRuntime runtime) {
		...
		if (placeable.filter(p -> !p.getTracedElement()
			...
			if (hidden.getValue().filter(e -> Objects.equals(e, true)).isPresent()) {
				...
				final var refId = placeableReference.getTracedElement().getRefId();
				final var result = new MarkupResult(
					refId,
					"",
					null,
					null,
		...
		return new RuntimeWalker<>(runtime).walkReference(
			new MarkupFactoryBuilder(dependency.getPrintDocumentContext()),
			dependency.getReference(),
			MarkupFactoryBuilder::single
		).apply(runtime);
	}
