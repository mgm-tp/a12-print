...
public class ConfirmTypeFormatter implements ValueFormatProvider {

	private boolean useCheckboxCheckedValue(final FormattingDetails details) {
		return (details.getDisplayType() == DisplayOptions.DisplayType.CHECKBOX)
			&& StringUtils.isNotBlank(details.getCheckboxCheckedValue());
	}
	...
	public @NonNull FormattingResult format(@NonNull final FormattingParameters parameters) {
		...
		var confirmTypeFormattingDetails = (FormattingDetails) parameters.formattingDetails();

		if (Objects.equals(value, true)
			|| ( value instanceof String && Constant.TRUE.getValue().equalsIgnoreCase((String) value))) {
			final var formattedValue = useCheckboxCheckedValue(confirmTypeFormattingDetails)
				? confirmTypeFormattingDetails.getCheckboxCheckedValue()
				: Boolean.TRUE.toString();
			return new FormattingResult(formattedValue, parameters.isHtml());
		}
		return new FormattingResult(StringUtils.EMPTY, parameters.isHtml());
	}
