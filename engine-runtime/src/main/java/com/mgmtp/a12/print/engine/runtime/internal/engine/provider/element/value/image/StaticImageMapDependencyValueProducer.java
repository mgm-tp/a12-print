...
public class StaticImageMapDependencyValueProducer
	implements CoreDependencyValueProvider<Map<String, byte[]>, StaticImageMapDependency> {

	@Override
	public ValueFactory<Map<String, byte[]>> produce(
		StaticImageMapDependency dependency,
		PrintJob job,
		PrintEngine<?> engine,
		InternalCorePrintEngineRuntime runtime
	) {
		final var compilationContext = (PrintModelCompilationContext)
			runtime.provide(new PrintModelDependency(job.getPrintModelId())).get();
		final Map<String, byte[]> staticImageMap = compilationContext.getStaticImageMap();
