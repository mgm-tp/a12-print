...
@Slf4j
public class ReferenceComponentDependencyValueProducer implements PdfBoxDependencyValueProvider<Component, ReferenceComponentDependency> {

	@Override
	public ValueFactory<Component> produce(ReferenceComponentDependency dependency, PrintJob job, PrintEngine<?> engine, InternalPdfBoxPrintEngineRuntime runtime) {
		...
		return new RuntimeWalker<>(runtime).walkReference(
			new ComponentFactoryBuilder(
				...
				dependency.getContainerWidth(),
				dependency.getTotalPageCount(),
				dependency.getCurrentPageCount()
			),
