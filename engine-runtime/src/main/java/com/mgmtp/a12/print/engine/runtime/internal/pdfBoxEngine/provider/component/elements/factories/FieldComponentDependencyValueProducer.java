...
public class FieldComponentDependencyValueProducer implements PdfBoxDependencyValueProvider<Component, FieldComponentDependency> {

	@Override
	public ValueFactory<Component> produce(FieldComponentDependency dependency, PrintJob job, PrintEngine<?> engine, InternalPdfBoxPrintEngineRuntime runtime) {
		final var formattingResult = runtime.provide(new FieldValueDependency(
			dependency.getFieldTrace().getTracedElement(),
			dependency.getPrintDocumentContext()
		)).get();

		return () -> new EntityComponent(
			dependency.getFieldTrace().getTracedElement().getId(),
			dependency.getFieldTrace(),
