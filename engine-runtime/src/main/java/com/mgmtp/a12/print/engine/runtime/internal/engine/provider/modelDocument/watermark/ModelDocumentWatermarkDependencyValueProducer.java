 ...
 */
package com.mgmtp.a12.print.engine.runtime.internal.engine.provider.modelDocument.watermark;
...
public class ModelDocumentWatermarkDependencyValueProducer implements ModelDocumentDependencyValueProvider<
	AttachmentWrapper<ModelDocumentWatermarkResult>,
	ModelDocumentWatermarkDependency
> {

	@Override
	public ValueFactory<AttachmentWrapper<ModelDocumentWatermarkResult>> produce(
		ModelDocumentWatermarkDependency dependency,
		PrintJob job,
		PrintEngine<?> engine,
		InternalModelDocumentPrintEngineRuntime runtime
	) {
		final var watermarkTrace = dependency.getWatermark();
		final var watermark = watermarkTrace.getTracedElement();
		...
		final var elements = runtime.streamReferenceModelDocumentDependency(
			ReferenceModelDocumentDependency.ofContainerReferences(
				watermarkTrace, printDocumentContext, Collections.emptyList()
			)
		...
		return () -> new AttachmentWrapper<>(
			new ModelDocumentWatermarkResult(
				watermark.getPageOrientation(),
				new PrintWatermark(
					watermark.getId(),
