...
import com.mgmtp.a12.print.engine.runtime.internal.manager.compiler.layout.*;
	...
	private static List<Spread> getCurrentSpreads(
		...
		return dependentReferenceIds.stream().map(dependentReferenceId -> {
				...
				if (dependentSpread == null) {
					throw new PrintException("Dependent reference spread for id {} not found", dependentReferenceId);
				}
