...
import com.mgmtp.a12.print.engine.runtime.internal.pdfBoxEngine.provider.component.ComponentFactoryBuilder;
import com.mgmtp.a12.print.engine.runtime.internal.runtime.RuntimeWalker;

public class ElementComponentDependencyDependencyValueProducer implements PdfBoxDependencyValueProvider<Component, ElementComponentDependency> {

	@Override
	public ValueFactory<Component> produce(ElementComponentDependency dependency, PrintJob job, PrintEngine<?> engine, InternalPdfBoxPrintEngineRuntime runtime) {
		return new RuntimeWalker<>(runtime).walkElement(
			new ComponentFactoryBuilder(
				dependency.getPrintDocumentContext(),
				dependency.getDocument(), null, dependency.getTotalPageCount(), dependency.getCurrentPageCount()),
			dependency.getElement(),
			ComponentFactoryBuilder::single
		).apply(runtime);
	}
}
