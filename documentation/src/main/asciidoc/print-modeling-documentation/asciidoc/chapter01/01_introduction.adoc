...
Follow the link:/docs/PRINT_ENGINE/print-technical-documentation/index.html#legacy_migration[print-shell migration instructions].
====

[WARNING]
====
When migrating a Print Model to a new version in the SME, any uncommitted changes stored in `.wal` files may become invalid after the migration.
Please commit all pending changes *before* opening and migrating the model in a newer SME version to avoid data loss.
====
...
The available Fonts are provided as workspace resources and can be selected in the Print Model when configuring <<TextStylesTab, Text Styles>>.
