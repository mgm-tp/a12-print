...
When a Print Model references an attachment, the print-shell resolves it by searching the workspace for a file whose *base name* (filename without extension) matches the attachment ID defined in the Print Model.
Only *PDF* and *image* files are supported as attachments.

.Example
If the Print Model references attachment ID `myReport`, place a file named `myReport.pdf` in the workspace directory.
The file will be loaded and embedded as the attachment during printing.
