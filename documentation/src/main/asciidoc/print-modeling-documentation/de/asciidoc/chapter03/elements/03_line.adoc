...
Das Line Element fügt eine horizontale Linie in das resultierende PDF-Dokument ein. Es funktioniert analog zum HTML-Element `<hr>`.
Die Eigenschaften des Line Elements werden durch seine <<BorderProperties, Border Properties>> bestimmt.

NOTE: Der *Border Style* eines Line-Elements ist immer erforderlich – die Quelle *Unset* ist nicht verfügbar.
Neue Line-Elemente werden standardmäßig mit dem *Border Style* *Solid* erstellt.