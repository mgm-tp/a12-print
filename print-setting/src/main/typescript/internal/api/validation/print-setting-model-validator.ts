...
import { PrintMetaModelValidationScript } from "../generated/validation/print-meta-model-validation-script.js";
import documentModelJson from "../generated/model/DomainPrintSettingMetaModel.json" with { type: "json" };

export class PrintSettingModelValidator extends PrintValidator {
	private static instance: PrintSettingModelValidator = new PrintSettingModelValidator();
	...
	public validate<T>(validatorInput: PrintValidator.Input): PrintValidator.IntegrityReport<T> {
		...
		if (transformedInput) {
			return fullValidation(transformedInput, this.documentRtService, this.documentModel);
		}

		return FAILED_INTEGRITY_REPORT;
	}

	public static getInstance(): PrintSettingModelValidator {
