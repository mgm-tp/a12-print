...
export interface Text extends PrintModelElement, Styleable {
	...
	readonly text: TextElementProperties;
}

export interface TextElementProperties extends ReferenceContainer<ReferenceContainerLabels.entities>, PrintModelEntity {
	readonly text: string;
	readonly hideIfEmpty?: boolean;
}

export namespace Text {
	export function isInstance(element: PartialAnyPrintModelElement): element is Text {
		return (
			element.type === ElementType.Text &&
			typeof element.text?.text === "string" &&
			Array.isArray(element.text.entities)
		);
	}
}
