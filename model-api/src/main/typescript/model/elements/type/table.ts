...
export interface TableProperties
	...
	readonly sumLabel: InputSource<string>;
	readonly headerTextProperties?: TextProperties;
}

export interface TableColumn extends PrintModelEntity {
	readonly label: InputSource<string>;
	readonly width: MeasureInputSource;
	readonly headerLabelHidden?: boolean;
	readonly sumColumn?: boolean;
	readonly hasCustomTextProperties?: boolean;
}

export namespace Table {
	export function isInstance(element: PartialAnyPrintModelElement): element is Table {
		return (
			element.type === ElementType.Table &&
			typeof element.table?.basePath === "string" &&
			Array.isArray(element.table.columns)
		);
	}
}
