...
public interface TextProperties extends PrintModelEntity {
	...
	Optional<StringInputSource> getColor();

	Optional<StringInputSource> getBackgroundColor();

	Optional<InputSource<Alignment>> getAlignment();

	Optional<BooleanInputSource> getBold();

	Optional<BooleanInputSource> getItalic();
	...
	enum Alignment implements InputSourceEnum {
		...
		@JsonProperty("Justify") JUSTIFY("Justify");

		private static final Map<String, Alignment> STRING_TO_ENUM = InputSourceEnum.buildStringToEnumMap(Alignment.class);

		@Getter
		private final String jsonValue;

		Alignment(String jsonPropertyValue) {
			this.jsonValue = jsonPropertyValue;
		}

		public static Alignment fromString(String text) {
