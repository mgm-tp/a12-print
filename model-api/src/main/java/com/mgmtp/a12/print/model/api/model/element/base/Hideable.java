...
@OnlyForUsage
public interface Hideable extends LogicContainer {
	List<HideCondition> getHideConditions();

	@Override
	default Stream<LogicComponent> logicComponents() {
		if(getHideConditions().isEmpty()){
			return Stream.empty();
		}
		final var id = getId() + "#" + getHideConditions().stream().map(PrintModelEntity::getId).collect(Collectors.joining("#"));
		return Stream.of(
			new LogicComponent() {
				@Override
				public Stream<String> computationStatements() {
					return getHideConditions().stream().map(HideCondition::getPrecondition);
				}
