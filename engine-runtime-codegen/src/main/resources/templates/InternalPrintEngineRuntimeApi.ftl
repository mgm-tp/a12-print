...
public interface Internal${engine.getRuntimeResultType()}PrintEngineRuntimeApi {

	<#list engine.getRuntimeDependencies() as dependency >
		ValueFactory<${dependency.getValue()}> provide(${dependency.getDependency()} dependency);
	</#list>

	<#list engine.getStreamDependencies() as dependency >
		Stream<${dependency.getValue()}> stream${dependency.getSimpleName()}(Stream<${dependency.getDependency()}> dependency);
	</#list>
