...
export const PrintTypesettingMigrationTool: MigrationTool = {
			...
			if (migrationResult.status === "success") {
				...
				if (!deserializedResult.result) {
					return {
						...
						status: "error",
						errorMessage: `Cannot deserialize the Typesetting model. Details: ${buildErrorFieldPaths(deserializedResult.report.errorMap["@error"])}`,
					};
				}

				const serializedResult = typesettingModelMarshaller.serialize(deserializedResult.result);

				if (!serializedResult.result) {
					return {
						model: migrationResult.model,
						status: "error",
						errorMessage: `Cannot serialize the Typesetting model. Details: ${buildErrorFieldPaths(serializedResult.report.errorMap["@error"])}`,
					};
				}
