			...
			if (migrationResult.status === "success") {
				...
				if (!deserializeResult.result) {
					return {
						...
						status: "error",
						errorMessage: `Cannot deserialize the print model. Details: ${buildErrorFieldPaths(deserializeResult.report.errorMap["@error"])}`,
					};
				}
				...
				if (!serializeResult.result) {
					return {
						model: migrationResult.model,
						status: "error",
						errorMessage: `Cannot serialize the print model. Details: ${buildErrorFieldPaths(serializeResult.report.errorMap["@error"])}`,
					};
				}

				return {
					...migrationResult,
					model: serializeResult.result,
				};
			}
