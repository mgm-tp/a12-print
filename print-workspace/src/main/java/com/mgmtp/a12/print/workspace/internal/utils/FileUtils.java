...
public class FileUtils {
	...
	public static String readFileContent(Path path) {
		try(InputStream inputStream = new FileInputStream(path.toString())) {
			...
			throw new PrintWorkspaceException("unable to read file", e);
		}
	}

	public static Optional<String> tryReadFileContent(Path path){
		try {
			return Optional.of(readFileContent(path));
		} catch (Exception e) {
