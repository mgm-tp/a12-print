...
class SyntaxTreeComputationFieldTypeClassifierTest {
	...
	@Test
	void classifyBooleanPredicate() {
		var classifier = SyntaxTreePredicateClassifier.load((a,v) ->  ComputationFieldType.UNKNOWN);
		var parser = new ComputationParser();
		var src = "FieldFilled(../relative/path1)";
		var result = parser.parseTree(src);

		var type = classifier.classifyFieldType(result.getRoot());

		assertThat(type).isEqualTo(ComputationFieldType.BOOLEAN);
	}

	@Test
	void classifyNumericArithmetic() {
		var classifier = SyntaxTreePredicateClassifier.load((a,v) ->  ComputationFieldType.UNKNOWN);
		var parser = new ComputationParser();
		var src = "1 + 2 + 3";
		var result = parser.parseTree(src);
	...
	@Test
	void classifyStringArithmetic() {
		var classifier = SyntaxTreePredicateClassifier.load((a,v) -> ComputationFieldType.STRING);
		var parser = new ComputationParser();
		var src = "[variable/field] + \"123\"";
		var result = parser.parseTree(src);
