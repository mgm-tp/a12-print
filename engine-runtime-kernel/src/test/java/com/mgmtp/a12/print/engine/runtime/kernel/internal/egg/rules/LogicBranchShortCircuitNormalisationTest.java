...
class LogicBranchShortCircuitNormalisationTest {
	...
	static Stream<RewriteRuleFactoryTestAssertion> positiveCases() {
		final var builder = RewriteRuleFactoryTestAssertion.StreamBuilder
			.builder()
			.rule(new LogicBranchShortCircuitNormalisation());
...

		{
			...
			builder.instance(
				RewriteRuleFactoryTestAssertion
					.Instance
					.builder()
					.name("does not apply to AND Branches without any False")
					.input(input)
		...
		}
		{
			...
			builder.instance(
				RewriteRuleFactoryTestAssertion
					.Instance
					.builder()
					.name("does not apply to OR Branches without any True")
					.input(input)
					.expectedOutput(input)
					.build()
			);
