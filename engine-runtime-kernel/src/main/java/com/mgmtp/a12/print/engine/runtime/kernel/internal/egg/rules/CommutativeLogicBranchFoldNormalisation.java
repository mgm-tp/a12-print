...
import com.mgmtp.a12.print.engine.runtime.kernel.internal.elements.Logic;
...
public class CommutativeLogicBranchFoldNormalisation implements EggRewriteRuleFactory {
	@Override
				...
				.logic((o, clone) ->
					builder -> {
						...
						if (Arrays.stream(clone.getBranches())
								  .allMatch(branch -> branch.elementType() != SyntaxTreeElementType.LOGIC
									  || !((Logic) branch).getOperator().equals(clone.getOperator()))) {
							return clone;
						}

						final var branches = new ArrayList<LogicBranch>(clone.getBranches().length);
						for (var branch : clone.getBranches()) {
							if (branch.elementType() == SyntaxTreeElementType.LOGIC
								&& ((Logic) branch).getOperator().equals(clone.getOperator())) {
								final var other = ((Logic) branch);
								Collections.addAll(branches, other.getBranches());
							} else {
								branches.add(branch);
							}
						}

						return builder.branches(branches.toArray(LogicBranch[]::new)).build();
					}
				)
				.build()
				.instantiate(e);
