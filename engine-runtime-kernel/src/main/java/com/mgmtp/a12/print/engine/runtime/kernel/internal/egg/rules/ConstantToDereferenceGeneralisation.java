...
public class ConstantToDereferenceGeneralisation implements EggRewriteRuleFactory {
	...
	private class Rewrite implements EggRewriteRuleInstance {
		...
		public Rewrite(SyntaxTreeElement e) {
			rewriteRule = RewriteSyntaxTree
				...
				.constant(
					(o, c) -> {
						final var constantNode = nodeFactory.apply(c);
						final var variable = variableFactory.apply(constantNode);
						return b -> Dereference.builder().variable(variable).build();
					}
				)
				.build()
				.instantiate(e);
