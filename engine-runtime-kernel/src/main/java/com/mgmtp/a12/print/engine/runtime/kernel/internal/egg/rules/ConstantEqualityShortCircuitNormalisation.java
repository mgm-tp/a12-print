...
public class ConstantEqualityShortCircuitNormalisation implements EggRewriteRuleFactory {
	...
	private static class Rewrite implements EggRewriteRuleInstance {
		...
		public Rewrite(SyntaxTreeElement e) {
			rewriteRule = RewriteSyntaxTree
				...
				.compare(
					(o, clone) -> {
						if (!clone.getOperator().isEqualityOrUnEquality()) {
							return b -> clone;
						}
						if (!Arrays.stream(clone.getBranches()).allMatch(
							branch -> branch.elementType().equals(SyntaxTreeElementType.CONSTANT))) {
							return b -> clone;
						}
						var branches = clone.getBranches();
						var left = (Constant) branches[0];
						var right = (Constant) branches[1];
						return clone.getOperator().equals(Compare.Operator.EQUALITY) == left.valueEquals(right)
							? b -> Constant.TRUE
							: b -> Constant.FALSE;
