...
public class IdentityEqualityCompareShortCircuitNormalisation implements EggRewriteRuleFactory {

	@Override
	public EggRewriteRuleInstance instantiate(SyntaxTreeElement e) {
		return new IdentityEqualityCompareShortCircuitNormalisation.Rewrite(e);
	}
			...
			rewriteRule = RewriteSyntaxTree
				...
				.compare(
					(o, clone) -> {
						...
						if (!clone.getOperator().entailsEqualitySemantic()) {
							return b -> clone;
						}

						final var branches = clone.getBranches();

						for (int i = 1; i < branches.length; i++) {
							if (!branches[i].equals(branches[0])) {
								return b -> clone;
							}
						}
