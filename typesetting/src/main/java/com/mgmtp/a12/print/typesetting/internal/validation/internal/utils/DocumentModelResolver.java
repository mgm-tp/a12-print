...
class DocumentModelResolver implements IDocumentModelResolver {

	private static final String TYPESETTING_MODEL_PATH = "/models/DomainTypesettingMetaModel.json";
	...
	public IDocumentModel getDocumentModelById(String id) {
		try {
			String model = ResourceFile.loadFileFromResources(TYPESETTING_MODEL_PATH);
			return documentModelSerializer.deserialize(new StringReader(model));
		} catch (IOException e) {
