...
export class CharTrieMap implements CharTrie {
	...
	private findIndex(char: string): number {
		return this.characters.findIndex(ch => ch === char);
	}

	put(ch: string, node: CharTrieMapNode): CharTrieMapNode | undefined {
		const oldIndex: number = this.findIndex(ch);
		if (oldIndex >= 0) {
			const oldValue: CharTrieMapNode = this.nodes[oldIndex];
			this.nodes[oldIndex] = node;
			return oldValue;
		}
