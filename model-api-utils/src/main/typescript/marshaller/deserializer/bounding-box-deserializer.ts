...
export class BoundingBoxDeserializer extends Deserializer<GeneratedDTO.BoundingBoxDTO, ModelAPI.BoundingBoxProperties> {
	prefix = "boundingBox";

	map(property: keyof GeneratedDTO.BoundingBoxDTO, dto: GeneratedDTO.BoundingBoxDTO) {
		switch (property) {
			case "id":
			...
			case "dimensions":
				return this.deserializeRequired(
					dto.dimensions,
					property,
					new BoundingBoxDimensionsDeserializer(this.path)
				);
...
class BoundingBoxDimensionsDeserializer extends Deserializer<
	GeneratedDTO.DimensionsDTO_2,
	ModelAPI.BoundingBoxDimensions
> {
	prefix = "dimensions";
	elementBase = {};
