...
export class MarginDeserializer extends Deserializer<GeneratedDTO.TopDTO, ModelAPI.Margin> {
	prefix = "margin";

	map(property: keyof GeneratedDTO.TopDTO, dto: GeneratedDTO.TopDTO) {
		switch (property) {
			case "id":
				return this.getRequired(dto.id, property);
			case "type":
				return this.getRequired(dto.type, property);
			case "margin":
				return this.deserializeRequired(dto.margin, property, new MeasureDeserializer(this.path));
		}
