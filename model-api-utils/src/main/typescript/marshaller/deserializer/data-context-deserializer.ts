...
export class DataContextDeserializer extends Deserializer<GeneratedDTO.DataContextsDTO, ModelAPI.DataContext> {
	prefix = "dataContext";

	map(property: keyof GeneratedDTO.DataContextsDTO, dto: GeneratedDTO.DataContextsDTO) {
		switch (property) {
			case "id":
				return dto.id;
			case "model":
			...
			case "isRepetition":
				return this.getRequired(dto.isRepetition, property);
		}
