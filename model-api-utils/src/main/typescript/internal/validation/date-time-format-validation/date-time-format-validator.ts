...
export class DateTimeFormatValidator {
	getValidationError(dateFormat: string): DateTimeFormatValidationError | undefined {
		const lexer = new DateTimeFormatLexer(CharStreams.fromString(dateFormat));
		const tokenStream = new CommonTokenStream(lexer);
		const parser = new DateTimeFormatParser(tokenStream);

		const errorListener = new DateTimeFormatErrorListener();
		parser.removeErrorListeners();
		parser.addErrorListener(errorListener);

		const tree = parser.entrypoint();

		const repetitionListener = new DateTimeFormatRepetitionListener();
		ParseTreeWalker.DEFAULT.walk(repetitionListener, tree);

		const parsingError = errorListener.getError();
		if (parsingError) {
			return parsingError;
		}

		const repetitionError = repetitionListener.getError();
		if (repetitionError) {
	...
	isValid(dateFormat: string): boolean {
		return !this.getValidationError(dateFormat);
	}
}
