...
export const ElementReferenceOverlapped: ICustomCondition = {
	check: function (
		...
		const { currentReference, references } = getValidationReferences(data.document, errorField);
...
function getValidationReferences(document: PrintModelDTO, errorField: IIdentifier) {
	const fieldPath = errorField.getName();
	const indices = errorField.getIndices();
