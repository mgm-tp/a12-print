...
export class PrintCustomConditionRegistry {
	private static instance = new PrintCustomConditionRegistry();
	...
	private registerOnce(): void {
		const factory: ICustomConditionFactory = {
			createCustomCondition: (name: string) =>
				this.getCustomConditionImplementation(name as CustomConditionName) ?? null,
		};
		CustomConditionService.getInstance().register(factory);
	}
	...
	public static getInstance(): PrintCustomConditionRegistry {
