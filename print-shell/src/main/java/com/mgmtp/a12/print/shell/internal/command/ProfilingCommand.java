...
public class ProfilingCommand {
	...
	@ShellMethod(value = "Profile Print Model")
	public void profiling(
		...
		@ShellOption(
			value = {"--iterations"},
			help = "Number if print times the PrintJob is executed for measurement",
			defaultValue = "100"
		) final Integer iterations,
		@ShellOption(
			value = {"--warmup"},
			help = "Number if print times the PrintJob is executed for warmup before measuring the iterations",
			defaultValue = "10"
		) final Integer warmupIterations,
		@ShellOption(
			value = {"--parallel"},
			help = "Execute all Iterations in parallel",
			defaultValue = "false"
		) final Boolean parallel,
		@ShellOption(
			value = {"--ongoingGCPressure"},
			help = "Request a GC and finalization after every iteration",
			defaultValue = "false"
		) final Boolean ongoingGCPressure
	) throws IOException {
		...
		printService.withLogSettingContext(
			...
			() -> {
				...
				try (final var snapshot = new PrintStream(new FileOutputStream(snapshotFile), true, StandardCharsets.UTF_8)) {
					...
					final var document = PDDocument.load(byteArray);
	...
	private PdfPrintEngine setupPrintEngine() {
