...
public class ShellExceptionResolver implements CommandExceptionResolver {
	...
	public CommandHandlingResult resolve(Exception ex) {
		if (ex instanceof PdfComparisonException) {
			return CommandHandlingResult.of(
				String.format(
					"The print shell comparison throws the exception \"%s\"", ex.getMessage()
				), 43);
		}
		if (ex instanceof PrintShellException) {
			return CommandHandlingResult.of(
				String.format(
					"The print shell throws the exception \"%s\"", ex.getMessage()
				), 42);
		}
		return null;
	}
